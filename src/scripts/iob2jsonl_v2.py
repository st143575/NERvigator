...
def segment(docid2word_tags: dict, iob2idx=None):
    """
    ...
    for _, words_tags in docid2word_tags.items():
        ...
        if iob2idx: sentences_tags['tag_ids'] = [[iob2idx[tag] for word, tag in sentence] for sentence in doc_sentences]
...
def convert_to_dataframe(segmented_documents):
    ...
    for doc_id, sentid2word_tags in enumerate(segmented_documents):
        ...
        if 'tag_ids' in sentid2word_tags.keys(): tag_ids = sentid2word_tags['tag_ids']
        
        assert len(sentences) == len(tags)
        
        for sent_id, sentence in enumerate(sentences):
            if 'tag_ids' in sentid2word_tags:
                data.append([doc_id, sent_id, sentence, tags[sent_id], tag_ids[sent_id]])
                columns = ["doc_id", "sent_id", "sentence", "tags", "tag_ids"]
            else: 
                data.append([doc_id, sent_id, sentence, tags[sent_id]])
                columns = ["doc_id", "sent_id", "sentence", "tags"]

    segmented_documents_df = pd.DataFrame(data, columns=columns)
    ...
    for subdir, dirs, files in os.walk(input_dir):
        ...
        for file in files:
            if file.endswith('.iob'):
                ...
                segmented_documents = segment(docid2word_tags, iob2idx=iob2idx[dataset_name])
